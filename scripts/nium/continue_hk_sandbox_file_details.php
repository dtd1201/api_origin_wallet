...
try {
    if (! $app->environment('staging') || ($argv[1] ?? '') !== '--execute=NIUM_HK_FILE_DETAILS_21_APPROVED') {
        throw new RuntimeException('Exact staging approval marker is required.');
    }

    $status = $app->make(NiumHkSandboxFileStageContinuation::class)->continueDocument21();
    ...
    fwrite(STDERR, 'NIUM_HK_FILE_DETAILS_EXIT_'.$exitCode.PHP_EOL);
    exit($exitCode);
}
