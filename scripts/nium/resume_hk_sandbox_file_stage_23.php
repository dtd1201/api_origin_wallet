...
try {
    if (! $app->environment('staging') || ($argv[1] ?? '') !== '--execute=NIUM_HK_FILE_RESUME_23_APPROVED') {
        throw new RuntimeException('Exact staging approval marker is required.');
    }
    $status = $app->make(NiumHkSandboxFileStageContinuation::class)->resumeDocument23();
    ...
    fwrite(STDERR, 'NIUM_HK_FILE_RESUME_23_EXIT_'.$exitCode.PHP_EOL);
    exit($exitCode);
}
