...
use App\Services\Nium\NiumHkStakeholderSubmitKycGenerationSixOneShotRunner;
...
class NiumHkStakeholderSubmitKycGenerationSixOneShotRunnerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_exact_three_field_set_passes_and_safe_audit_is_immutable(): void
    {
        ...
        $this->assertSame(116, $result['previous_log_id']);
        ...
        $this->assertSame(0, $result['g6_post_count']);
        ...
        $this->assertArrayNotHasKey('nium_stakeholder_submit_kyc_retry_generation_6', UserProviderAccount::query()->findOrFail(7)->metadata);
    }

    public function test_g6_manual_identity_and_poa_contract(): void
    {
        ...
        $this->assertArrayHasKey('isResident', $payload);
        $this->assertFalse($payload['isResident']);
    }

    public function test_exact_generation_five_rejection_and_tenant_webhook_evidence_pass(): void
    {
        $result = $this->runner()->audit();
        ...
        $this->assertTrue($result['is_resident_present']);
        $this->assertFalse($result['is_resident']);
    }

    public function test_wrong_generation_five_rejection_evidence_fails(): void
    {
        foreach ([
            ['response_status' => 422],
            ...
            $log = ApiRequestLog::query()->findOrFail(116);
    ...
    public function test_wrong_generation_five_error_item_shape_fails(): void
    {
        foreach ([
            ['error_field' => 'kycMode'],
            ...
            $this->assertAuditHolds();
            $this->seedEntityTypeResponseBody($log);
        }
    }
    ...
    public function test_generation_six_exact_contract_is_enforced(): void
    {
        $invalid = [];
        $missing = $this->payload();
        unset($missing['isResident']);
        $invalid[] = $missing;
        foreach ([true, 'false', 0] as $value) {
            $invalid[] = [...$this->payload(), 'isResident' => $value];
        }
        $invalid[] = [...$this->payload(), 'entityType' => 'INDIVIDUAL_STAKEHOLDER'];
        $invalid[] = [...$this->payload(), 'kycMode' => 'MANUAL_KYC'];

        foreach ($invalid as $payload) {
            try {
                app(NiumHkSubmitKycValidator::class)->assertManualGenerationSix($payload);
                $this->fail('Invalid generation #6 exact contract was accepted.');
    ...
    {
        ApiRequestLog::query()->findOrFail(114)->forceFill(['id' => 117])->save();
        $this->assertAuditHolds();
    }
    ...
    public function test_generation_five_rejected_claim_and_no_g6_claim_are_required(): void
    {
        $account = UserProviderAccount::query()->findOrFail(7);
        $metadata = $account->metadata;
        $metadata['nium_stakeholder_submit_kyc_retry_generation_5']['state'] = 'unknown';
        $account->forceFill(['metadata' => $metadata])->save();
        $this->assertAuditHolds();
    }

    public function test_existing_g6_claim_blocks(): void
    {
        $account = UserProviderAccount::query()->findOrFail(7);
        $metadata = $account->metadata;
        $metadata['nium_stakeholder_submit_kyc_retry_generation_6'] = ['state' => 'submitting'];
        $account->forceFill(['metadata' => $metadata])->save();
        $this->assertAuditHolds('HOLD_GENERATION_6_ALREADY_CLAIMED');
    }
    ...
    public function test_transport_unknown_is_one_shot(): void
    {
        ...
        $this->mock(NiumService::class, function (MockInterface $mock): void {
            ...
            $mock->shouldReceive('post')->once()->andReturnUsing(function (): never {
                $this->assertSame('submitting', $this->g6State());
    ...
    private function runMocked(int $status, array $body): array
    {
        ...
        $this->mock(NiumService::class, function (MockInterface $mock) use ($status, $body): void {
            ...
            $mock->shouldReceive('post')->once()->andReturnUsing(function () use ($status, $body): Response {
                $this->assertSame('submitting', $this->g6State());
                $claim = UserProviderAccount::query()->findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_6'];
                ...
                $this->createLog(117, self::STAKEHOLDER_REFERENCE, $status, $status < 400, $body);
                return new Response(new \GuzzleHttp\Psr7\Response($status, [], json_encode($body, JSON_THROW_ON_ERROR)));
            });
        });
        $result = $this->runner()->run(true);
        $this->assertSame(1, $result['g6_post_count']);
        $this->assertSame($before, $this->immutableEvidence());
        return $result;
    }
    ...
    {
        $this->assertSame($state, $this->g6State());
        $this->assertSame($posts, ApiRequestLog::query()->where('id', '>', 116)->where('operation', 'submit_kyc')->count());
        try {
            $this->runner()->run(true);
            $this->fail('Expected G6 claim to block second run.');
        } catch (RuntimeException $exception) {
            $this->assertSame('HOLD_GENERATION_6_ALREADY_CLAIMED', $exception->getMessage());
        }
        $this->assertSame($posts, ApiRequestLog::query()->where('id', '>', 116)->where('operation', 'submit_kyc')->count());
    ...
    {
        return app(NiumHkSubmitKycPayloadFactory::class)->buildManualGenerationSix(
    ...
    private function g6State(): ?string
    {
        return UserProviderAccount::query()->findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_6']['state'] ?? null;
    }
    ...
    private function assertAuditHolds(?string $message = null): void
    {
        try {
            $this->runner()->audit();
            $this->fail('Expected G6 audit hold.');
        } catch (RuntimeException $exception) {
            if ($message !== null) {
    ...
    private function runner(): NiumHkStakeholderSubmitKycGenerationSixOneShotRunner
    {
        return app(NiumHkStakeholderSubmitKycGenerationSixOneShotRunner::class);
    }
    ...
    private function seedEvidence(): void
    {
        ...
        UserProviderAccount::query()->forceCreate(['id' => 7, 'user_id' => 9, 'provider_id' => 1,
            ...
            'metadata' => [
                'nium_stakeholder_submit_kyc_retry_generation_4' => ['state' => 'rejected'],
                'nium_stakeholder_submit_kyc_retry_generation_5' => ['state' => 'rejected'],
            ]]);
        ...
        $this->createLog(116, self::STAKEHOLDER_REFERENCE, 400, false, [
            'error_code' => 'invalid_input',
            'error_field_fingerprint' => 'b4753588f3f6ef2b',
            'error_items' => [[
                'error_code' => 'invalid_input',
    ...
    private function seedEntityTypeResponseBody(ApiRequestLog $log): void
    {
