...
class AdminPingPongTransferStatusApiTest extends TestCase
{
    use RefreshDatabase;

    public function test_admin_can_sync_pingpong_transfer_status_via_api(): void
    {
        Cache::flush();
        ...
        config()->set('services.pingpong.auth', [
            ...
            'cache_key' => 'tests:pingpong:admin-transfer-status-api:token',
        ...
        $response = $this->withToken($this->issueTokenFor($admin))
            ->postJson("/api/admin/transfers/{$transfer->id}/sync-status");
