...
use App\Services\Nium\NiumHkStakeholderSubmitKycGenerationFourOneShotRunner;
...
class NiumHkStakeholderSubmitKycGenerationFourOneShotRunnerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_exact_three_field_set_passes_and_safe_audit_is_immutable(): void
    {
        ...
        $this->assertSame(114, $result['generation_three_log_id']);
        $this->assertSame(3, $result['generation_three_error_field_count']);
        ...
        $this->assertSame(0, $result['g4_post_count']);
        ...
        $this->assertArrayNotHasKey('nium_stakeholder_submit_kyc_retry_generation_4', UserProviderAccount::query()->findOrFail(7)->metadata);
    }

    public function test_g4_manual_identity_and_poa_contract(): void
    {
    ...
    {
        ApiRequestLog::query()->findOrFail(114)->forceFill(['id' => 115])->save();
        $this->assertAuditHolds();
    }
    ...
    public function test_generation_three_rejected_claim_and_no_g4_claim_are_required(): void
    {
        $account = UserProviderAccount::query()->findOrFail(7);
        $metadata = $account->metadata;
        $metadata['nium_stakeholder_submit_kyc_retry_generation_3']['state'] = 'unknown';
        $account->forceFill(['metadata' => $metadata])->save();
        $this->assertAuditHolds();
    }

    public function test_existing_g4_claim_blocks(): void
    {
        $account = UserProviderAccount::query()->findOrFail(7);
        $metadata = $account->metadata;
        $metadata['nium_stakeholder_submit_kyc_retry_generation_4'] = ['state' => 'submitting'];
        $account->forceFill(['metadata' => $metadata])->save();
        $this->assertAuditHolds('HOLD_GENERATION_4_ALREADY_CLAIMED');
    }
    ...
    public function test_transport_unknown_is_one_shot(): void
    {
        ...
        $this->mock(NiumService::class, function (MockInterface $mock): void {
            ...
            $mock->shouldReceive('post')->once()->andReturnUsing(function (): never {
                $this->assertSame('submitting', $this->g4State());
    ...
    private function runMocked(int $status, array $body): array
    {
        ...
        $this->mock(NiumService::class, function (MockInterface $mock) use ($status, $body): void {
            ...
            $mock->shouldReceive('post')->once()->andReturnUsing(function () use ($status, $body): Response {
                $this->assertSame('submitting', $this->g4State());
                $claim = UserProviderAccount::query()->findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_4'];
                ...
                $this->createLog(115, self::STAKEHOLDER_REFERENCE, $status, $status < 400, $body);
                return new Response(new \GuzzleHttp\Psr7\Response($status, [], json_encode($body, JSON_THROW_ON_ERROR)));
            });
        });
        $result = $this->runner()->run(true);
        $this->assertSame(1, $result['g4_post_count']);
        $this->assertSame($before, $this->immutableEvidence());
        return $result;
    }
    ...
    {
        $this->assertSame($state, $this->g4State());
        $this->assertSame($posts, ApiRequestLog::query()->where('id', '>', 114)->where('operation', 'submit_kyc')->count());
        try {
            $this->runner()->run(true);
            $this->fail('Expected G4 claim to block second run.');
        } catch (RuntimeException $exception) {
            $this->assertSame('HOLD_GENERATION_4_ALREADY_CLAIMED', $exception->getMessage());
        }
        $this->assertSame($posts, ApiRequestLog::query()->where('id', '>', 114)->where('operation', 'submit_kyc')->count());
    ...
    private function g4State(): ?string
    {
        return UserProviderAccount::query()->findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_4']['state'] ?? null;
    }
    ...
    private function assertAuditHolds(?string $message = null): void
    {
        try {
            $this->runner()->audit();
            $this->fail('Expected G4 audit hold.');
        } catch (RuntimeException $exception) {
            if ($message !== null) {
    ...
    private function runner(): NiumHkStakeholderSubmitKycGenerationFourOneShotRunner
    {
        return app(NiumHkStakeholderSubmitKycGenerationFourOneShotRunner::class);
    }
    ...
    private function seedEvidence(): void
    {
        ...
        UserProviderAccount::query()->forceCreate(['id' => 7, 'user_id' => 9, 'provider_id' => 1,
            ...
            'metadata' => ['nium_stakeholder_submit_kyc_retry_generation_3' => ['state' => 'rejected']]]);
        ...
        $this->createLog(114, self::STAKEHOLDER_REFERENCE, 400, false, ['error_items' => array_map(
