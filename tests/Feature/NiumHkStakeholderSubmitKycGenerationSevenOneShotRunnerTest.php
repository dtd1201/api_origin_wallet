...
use App\Services\Nium\NiumHkStakeholderSubmitKycGenerationSevenOneShotRunner;
...
class NiumHkStakeholderSubmitKycGenerationSevenOneShotRunnerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_exact_g7_payload_and_historical_distinctions(): void
    {
        ...
        $g7 = [$payload['entityType'], $payload['kycMode'], array_key_exists('isResident', $payload)];
        $this->assertNotSame(['INDIVIDUAL_STAKEHOLDER', 'MANUAL_KYC', false], $g7);
        $this->assertNotSame(['individual_stakeholder', 'manual_kyc', false], $g7);
        $this->assertNotSame(['individual_stakeholder', 'manual_kyc', true], $g7);
    }
    ...
    public function test_offline_audit_reports_session_117_conflict_without_writes(): void
    {
        ...
        $this->assertSame(0, $result['g7_post_count']);
        $this->assertNull($result['g7_claim_state']);
    ...
    public function test_session_117_blocks_before_claim_and_http(): void
    {
        ...
        $this->assertArrayNotHasKey('nium_stakeholder_submit_kyc_retry_generation_7', UserProviderAccount::findOrFail(7)->metadata);
        $this->assertSame(5, $this->stakeholderPosts());
    }
    ...
    public function test_entity_completion_evidence_is_not_authoritative_for_customer_completion(): void
    {
        ...
        $this->assertSame(1, $result['g7_post_count']);
    }

    public function test_ambiguous_historical_claim_and_any_g6_claim_block_before_g7_claim(): void
    {
        foreach (['historical_unknown', 'g6_present'] as $mutation) {
            ...
            if ($mutation === 'historical_unknown') {
                ...
                $metadata['nium_stakeholder_submit_kyc_retry_generation_6'] = ['state' => 'rejected'];
            }
            $account->forceFill(['metadata' => $metadata])->save();

            try {
                $this->runner()->run(true, true);
                $this->fail("Expected {$mutation} to block G7.");
            } catch (RuntimeException $exception) {
                $this->assertStringContainsString(
                    $mutation === 'historical_unknown' ? 'ambiguous or inconsistent' : 'never executed',
    ...
    public function test_claim_precedes_single_post_and_uses_new_generation_key(): void
    {
        ...
        $this->mockResponse(200, [
            ...
            $this->assertSame('submitting', $metadata['nium_stakeholder_submit_kyc_retry_generation_7']['state']);
            $this->assertSame(7, $metadata['nium_stakeholder_submit_kyc_retry_generation_7']['generation']);
            $this->assertSame(117, $metadata['nium_stakeholder_submit_kyc_retry_generation_7']['prebuilt_session_log_id']);
            $this->assertSame(
                'human_verified_provider_ui_expired',
                $metadata['nium_stakeholder_submit_kyc_retry_generation_7']['expired_prebuilt_session_override'],
            );
            $this->assertArrayNotHasKey('nium_stakeholder_submit_kyc_retry_generation_6', $metadata);
            $this->assertSame('INDIVIDUAL_STAKEHOLDER', $payload['entityType']);
            $this->assertSame('manual_kyc', $payload['kycMode']);
            $this->assertArrayNotHasKey('isResident', $payload);
        });
    ...
    public function test_locked_claim_rebuilds_payload_and_persists_its_exact_fingerprint(): void
    {
        ...
        $this->assertSame(
            $fingerprint,
            UserProviderAccount::findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_7']['contract_fingerprint'],
        );
    }
    ...
    public function test_4xx_5xx_and_malformed_response_permanently_block_replay(): void
    {
        foreach ([[400, []], [500, []], [200, []]] as [$status, $body]) {
            ...
            }, UserProviderAccount::findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_7']['state']);
            $this->assertReplayBlocked(1);
            $this->resetG7();
        }
    }
    ...
    public function test_locked_claim_revalidates_preliminary_state_before_http(string $mutation): void
    {
        ...
        $claimMetadata = UserProviderAccount::findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_7'] ?? null;
        if ($mutation === 'concurrent_g7_claim') {
        ...
        $this->assertSame(5 + ($mutation === 'extra_history' ? 1 : 0), $this->stakeholderPosts());
    }
    ...
    public static function atomicMutationProvider(): array
    {
        return [
            ...
            'concurrent G7 claim' => ['concurrent_g7_claim'],
    ...
    private function mockResponse(int $status, array $body, ?callable $assertPayload = null): void
    {
        $this->mock(NiumService::class, function (MockInterface $mock) use ($status, $body, $assertPayload): void {
            ...
            $mock->shouldReceive('post')->once()->andReturnUsing(function (string $path, array $payload, User $user, $related, string $operation, string $reference) use ($status, $body, $assertPayload): Response {
                ...
                $this->createLog(118, self::REFERENCE, $status, $status < 400, $body);
    ...
    private function mockThrowable(\Throwable $throwable): void
    {
        $this->mock(NiumService::class, function (MockInterface $mock) use ($throwable): void {
            ...
            $mock->shouldReceive('post')->once()->andReturnUsing(function () use ($throwable): never {
                $this->assertSame('submitting', UserProviderAccount::findOrFail(7)->metadata['nium_stakeholder_submit_kyc_retry_generation_7']['state']);
                throw $throwable;
            });
        });
    }
    ...
    private function assertReplayBlocked(int $posts): void
    {
        try {
            $this->runner()->run(true, true);
            $this->fail('Expected G7 replay block.');
        } catch (RuntimeException $exception) {
            $this->assertSame('HOLD_GENERATION_7_ALREADY_CLAIMED', $exception->getMessage());
        }
        $this->assertSame(5 + $posts, $this->stakeholderPosts());
    ...
    private function assertCompletionEvidenceBlocksG7(): void
    {
        try {
            $this->runner()->run(true, true);
            $this->fail('Expected authoritative customer completion evidence to block G7.');
        } catch (RuntimeException $exception) {
    ...
    {
        ApiRequestLog::query()->where('id', '>', 117)->delete();
        $account = UserProviderAccount::findOrFail(7);
        $metadata = $account->metadata;
        unset($metadata['nium_stakeholder_submit_kyc_retry_generation_7']);
        $account->forceFill(['metadata' => $metadata])->save();
    }
    ...
    private function applyAtomicMutation(string $mutation): void
    {
        ...
        if ($mutation === 'extra_history') {
            $this->createLog(118, self::REFERENCE, 400, false, ['error_code' => 'invalid_input']);
            return;
        }
        ...
        $metadata = $account->metadata;
        if ($mutation === 'concurrent_g7_claim') {
            $metadata['nium_stakeholder_submit_kyc_retry_generation_7'] = ['state' => 'submitting', 'owner' => 'concurrent'];
    ...
    {
        return app(NiumHkSubmitKycPayloadFactory::class)->buildManualGenerationSeven(
            KycRelatedPerson::findOrFail(14), self::REFERENCE,
            KycDocument::findOrFail(27), KycDocument::findOrFail(28),
        );
    }

    private function runner(): NiumHkStakeholderSubmitKycGenerationSevenOneShotRunner
    {
        return app(NiumHkStakeholderSubmitKycGenerationSevenOneShotRunner::class);
    }
    ...
    {
        return [
            'logs' => ApiRequestLog::query()->whereIn('id', [104, 106, 113, 114, 115, 116, 117])->orderBy('id')->get()->map->getRawOriginal()->all(),
