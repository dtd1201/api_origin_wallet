...
final class NiumHkStakeholderSubmitKycGenerationSixOneShotRunner
{
    private const REQUIRED_PARENT_HEAD = '2c33f2a3daedab0995d128600c6d20d9e3a69b81';
    ...
    private const G5_LOG_ID = 116;
    private const G5_CLAIM = 'nium_stakeholder_submit_kyc_retry_generation_5';
    private const CLAIM_KEY = 'nium_stakeholder_submit_kyc_retry_generation_6';
    ...
    public function audit(): array
    {
        ...
        return [
            ...
            'previous_log_id' => self::G5_LOG_ID,
            ...
            'is_resident' => $context['payload']['isResident'],
            'g6_post_count' => 0,
            'db_write_count' => 0,
        ];
    }
    ...
    public function run(bool $separateHumanApproval = false): array
    {
        if (! $separateHumanApproval) {
            throw new RuntimeException('Separate human approval is required outside the generation #6 runner.');
        }
        $context = $this->preflight();
    ...
    private function preflight(): array
    {
        if (! $this->currentHeadIsCompatible()) {
            throw new RuntimeException('The deployed Git HEAD does not include the locked pre-G6 lineage.');
        }
        if (substr(hash('sha256', self::ERROR_FIELD), 0, 16) !== self::ERROR_FIELD_FINGERPRINT) {
            throw new RuntimeException('Generation #6 error-field fingerprint invariant failed.');
        }
        ...
        if (($metadata[self::G5_CLAIM]['state'] ?? null) !== 'rejected') {
            throw new RuntimeException('Generation #5 rejected claim is required.');
        }
        if (array_key_exists(self::CLAIM_KEY, $metadata)) {
            throw new RuntimeException('HOLD_GENERATION_6_ALREADY_CLAIMED');
        }
        ...
        $payload = $this->payloadFactory->buildManualGenerationSix(
    ...
    private function assertHistory(int $providerId): void
    {
        ...
        $this->assertGenerationFourRejection(ApiRequestLog::query()->findOrFail(115), $providerId);
        $this->assertGenerationFiveRejection(ApiRequestLog::query()->findOrFail(self::G5_LOG_ID), $providerId);
        ...
        if ($ids !== [106, 113, 114, 115, 116]) {
            throw new RuntimeException('Logs #106, #113, #114, #115, and #116 must be the sole stakeholder Submit KYC POSTs.');
        }
    }
    ...
    private function assertGenerationFiveRejection(ApiRequestLog $log, int $providerId): void
    {
        $body = (array) $log->response_body;
        $items = $body['error_items'] ?? null;
        if (! $this->historicalScopeIsValid($log, $providerId, self::G5_LOG_ID)
            ...
            throw new RuntimeException('Locked generation #5 rejection evidence #116 is invalid.');
        }
    }
    ...
    private function assertGenerationFourRejection(ApiRequestLog $log, int $providerId): void
    {
        $body = (array) $log->response_body;
        $items = $body['error_items'] ?? null;
        if (! $this->historicalScopeIsValid($log, $providerId, 115)
            || ($body['error_field_fingerprint'] ?? null) !== self::ERROR_FIELD_FINGERPRINT
            || ! is_array($items) || ! array_is_list($items) || count($items) !== 1
            || ($items[0]['error_code'] ?? null) !== 'invalid_input'
            || ($items[0]['error_field'] ?? null) !== self::ERROR_FIELD
    ...
    private function claim(): void
    {
        DB::transaction(function (): void {
            ...
            $metadata = (array) $account->metadata;
            if (($metadata[self::G5_CLAIM]['state'] ?? null) !== 'rejected' || array_key_exists(self::CLAIM_KEY, $metadata)) {
                throw new RuntimeException('Generation #6 claim precondition failed.');
            }
            $metadata[self::CLAIM_KEY] = [
                'state' => 'submitting', 'previous_log_id' => self::G5_LOG_ID, 'previous_http_status' => 400,
    ...
    private function finish(string $terminal, array $context, int $logMaxId): array
    {
        ...
        if ($posts > 1 || ($terminal === 'PASS_KYC_INITIATED' && $posts !== 1)) {
            throw new RuntimeException('Stakeholder generation #6 postcondition failed closed.');
        }

        return ['terminal' => $terminal, 'g6_post_count' => $posts];
    }
