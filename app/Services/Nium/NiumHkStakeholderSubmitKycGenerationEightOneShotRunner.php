...
final class NiumHkStakeholderSubmitKycGenerationEightOneShotRunner
{
    ...
    private const CLAIM_KEY = 'nium_stakeholder_submit_kyc_retry_generation_8';
    private const G7_CLAIM_KEY = 'nium_stakeholder_submit_kyc_retry_generation_7';
    private const SESSION_CLAIM_KEY = 'nium_kyc_prebuilt_form_session';
    private const HISTORICAL_LOG_IDS = [106, 113, 114, 115, 116, 118];
    ...
    public function audit(): array
    {
        ...
        return [
            ...
            'checkpoint' => 'OFFLINE_G8_CURRENT_DOCS_CANDIDATE',
            'historical_latest_submit_log' => 118,
            'historical_submit_count' => 6,
            'g7_claim_state' => $context['g7_claim_state'],
            'g7_provider_evidence_locked' => true,
            'g8_claim_state' => $context['g8_claim_state'],
            'g8_post_count' => 0,
            ...
            'provider_risk' => 'pre-built session coexistence remains unconfirmed; NO_AUTOMATIC_G9',
        ];
    }
    ...
    ): array
    {
        if (! $separateHumanApproval) {
            throw new RuntimeException('Separate human approval is required outside the generation #8 runner.');
        }
    ...
    private function preflight(bool $forExecution, bool $expiredSessionOverride = false): array
    {
        ...
        if (array_key_exists(self::CLAIM_KEY, $metadata)) {
            throw new RuntimeException('HOLD_GENERATION_8_ALREADY_CLAIMED');
        }
        ...
        $payload = $this->payloadFactory->buildManualGenerationEight(
        ...
        return [
            ...
            'g7_claim_state' => $metadata[self::G7_CLAIM_KEY]['state'] ?? null,
            'g8_claim_state' => $metadata[self::CLAIM_KEY]['state'] ?? null,
        ];
    }
    ...
    private function assertHistory(int $providerId): void
    {
        ...
        if ($ids !== self::HISTORICAL_LOG_IDS) {
        ...
        $this->assertGenerationSevenProviderEvidence(ApiRequestLog::query()->findOrFail(118));
    }
    ...
    private function assertDistinctContract(array $payload): void
    {
        ...
        if ($tuple !== ['individual_stakeholder', 'MANUAL_KYC', false]
            ...
            throw new RuntimeException('Generation #8 contract is not historically distinct.');
        }
    }
    ...
    private function claim(array $preliminary): array
    {
        return DB::transaction(function () use ($preliminary): array {
            ...
            if ((int) $account->provider_id !== (int) $provider->id || (int) $account->user_id !== self::USER_ID
                ...
                throw new RuntimeException('Account 7 changed after preliminary G8 preflight.');
            }
            $metadata = (array) $account->metadata;
            $this->assertHistoricalClaims($metadata);
            if (array_key_exists(self::CLAIM_KEY, $metadata)) {
                throw new RuntimeException('Generation #8 claim precondition failed.');
            }
            ...
            if (! hash_equals($preliminary['payload_fingerprint'], $payloadFingerprint)) {
                throw new RuntimeException('G8 payload changed after preliminary preflight.');
            }
            ...
            $metadata[self::CLAIM_KEY] = [
                'state' => 'submitting', 'generation' => 8, 'contract_fingerprint' => $payloadFingerprint,
    ...
    private function assertLockedDocument(
        ...
        if (! $document instanceof KycDocument) {
            throw new RuntimeException('Locked G8 documents 27 and 28 are required.');
        }
        ...
        if ((int) $document->kyc_profile_id !== self::PROFILE_ID
            ...
            throw new RuntimeException('Locked G8 documents 27 and 28 must be factual, approved, and NIUM AVAILABLE.');
        }
    }
    ...
    private function assertLockedHistory(int $providerId): void
    {
        ...
        $this->assertGenerationSevenProviderEvidence($logs->last());
    }
    ...
    private function assertHistoricalClaims(array $metadata): void
    {
        ...
        $g7 = $metadata[self::G7_CLAIM_KEY] ?? null;
        if (! is_array($g7)
            || ($g7['state'] ?? null) !== 'rejected'
            || ($g7['generation'] ?? null) !== 7
            || ($g7['provider_http_status'] ?? null) !== 400
            || ($g7['transport_outcome'] ?? null) !== 'response_received') {
            throw new RuntimeException('Historical generation #7 rejected claim is required.');
        }
    }

    private function assertGenerationSevenProviderEvidence(ApiRequestLog $log): void
    {
        $body = (array) $log->response_body;
        $items = is_array($body['error_items'] ?? null) ? $body['error_items'] : [];
        $fieldEvidence = ($body['error_field'] ?? null) === 'entityType'
            || ($body['error_field_fingerprint'] ?? null) === 'b4753588f3f6ef2b'
            || collect($items)->contains(static fn (mixed $item): bool => is_array($item)
                && (($item['error_field'] ?? null) === 'entityType'
                    || ($item['error_field_fingerprint'] ?? null) === 'b4753588f3f6ef2b'));
        $descriptionEvidence = ($body['error_description_fingerprint'] ?? null) === '0ae1eaf6418f92d1'
            || collect($items)->contains(static fn (mixed $item): bool => is_array($item)
                && ($item['error_description_fingerprint'] ?? null) === '0ae1eaf6418f92d1');
        if ((int) $log->id !== 118 || $log->operation !== 'submit_kyc' || $log->request_method !== 'POST'
            || $log->external_reference !== self::REFERENCE_ID || (int) $log->response_status !== 400
            || $log->is_success || $log->transport_outcome !== 'response_received'
            || ($body['error_code'] ?? null) !== 'invalid_input' || ! $fieldEvidence || ! $descriptionEvidence) {
            throw new RuntimeException('HOLD_G7_EVIDENCE_MISMATCH');
        }
    }
    ...
    private function finish(string $terminal, array $context, int $logMaxId, int $applicantPosts): array
    {
        ...
        return [
            'terminal' => $terminal,
            'g8_post_count' => $posts,
            'applicant_submit_kyc_post_count_change' => 0,
            'next_generation' => 'NO_AUTOMATIC_G9',
        ];
    }
